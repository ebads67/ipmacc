            ...
            for var in self.oacc_kernelsComp[kernelId]:
                for arg in args:
                    if arg.replace('*',' ').split()[-1]==str(var):
                        ...
                        compCodeInit+='#define '+str(var)+'(index) decompress_'+arg.replace('*',' ').split()[0]+'((void*)'+str(var)+', index, __compress_constant_main_'+str(var)+')\n'
        ...
        if self.target_platform=='CUDA':
            if len(self.oacc_kernelsComp)>0:
                ...
                self.code+="__device__ __inline unsigned short compress_float(float fNum, float* coef){\n\
                 float temp;\n\
                 temp = fNum/coef[0]+1.5;\n\
                 return (unsigend short)(((*((unsigned int*)&temp))>>7)&0x0000FFFF);\n}\n"
                self.code+="__device__ __inline float decompress_float(void *ptr,int index,float* coef){\n\
                 unsigned int temp;\n\
                 temp=((unsigned short*)ptr)[index];\n\
                 temp=temp<<7;\n\
                 return ((*((float*)&(temp=temp | 0x3F800000))-1.5)* coef[0]);\n}\n"
